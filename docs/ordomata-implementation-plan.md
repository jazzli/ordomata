...
A third fixed Class 1 PEP covers only a local deterministic-mock attempt claim:
it binds the queued source and running target revisions, immutable flow and
attempt inputs, initial attempt/flow event references, control revision, and
digest-only controller/lease references; persists and rereads its permit before
...
General/live/comparison and supervisor worker-dispatch admission, shared
...
The thirty-ninth bounded Phase 3 slice adds a separate library-only schema-v1
Class 0 direct-dependency declaration primitive over the same active detached
descriptor lease. It reads only bounded ELF `DT_NEEDED` and thin Mach-O
dylib-load syntax and keeps declarations digest-only. It does not resolve a
name, open a path, load a target, establish recursive/shared-library closure,
...
contiguous frozen v1-v12 identity prefix, and agreement between the recorded
...
completion-shadow, post-v11 expired-claim reconciliation-shadow, and post-v12
queued-deadline reconciliation-shadow coverage, alongside order, schema guards,
and migration provenance. The v10 shadow is
...
pre-dispatch terminal repair with redacted inactive-lease evidence; the v12
shadow separately binds a deterministic queued-deadline terminal repair and
its local outbox. None delivers the outbox nor enables execution. The loop
deliberately uses the shared verified migration ledger; missing v2-v12 schema
