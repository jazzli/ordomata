...
contract and the public guard are unchanged. The nineteenth slice freshly
consumes their evidence at its own separate Class 1 effect boundary.

Implemented as the nineteenth bounded Phase 3 slice, the separate library-only
schema-v1 `ordomata.repository_executable_shebang_nested_target_staging` API
exposes `stage_repository_executable_shebang_nested_target_bytes(
...
This Class 1 primitive requires exact expected nested-resolution and known-
chain-guard receipts, the complete active same-PID source- and target-stage
receipt/lease lineages, the exact nested-target path expectation, a freshly revalidated
...
semantics, the action-boundary guard replay passes each unique depth-2 target's
same still-pinned guarded descriptor to a bounded capture sink without changing
its metadata, identity, flags, offset, or inheritance state. The action guard
must exactly reproduce the expected guard. After the local effect, a complete
guard replay must again reproduce both while the exact source and target leases
...
The caller-supplied nested staging root must be an exact concrete absolute,
effective-user-owned mode-`0700`, empty directory. Freshly pinned repository,
search, source-stage, and target-stage roots remain identity-stable and
distinct; path overlap with those roots or an expected nested-target path is
rejected, and no expected target ancestor may alias the staging-root identity.
Every unique target is copied once into an exclusive no-follow temporary
regular file. Its name is unlinked and its directory synchronized before any
captured bytes are written. The file is changed to mode `0400`, synchronized,
independently read back in full, and retained only by a non-inheritable
`O_RDONLY` descriptor after writer closure. Shared targets preserve exact
ordered requirement and command correspondence. Source-native and target-
native chains return an active zero-file lease without inspecting or mutating
the nested staging root.
...
expected/action/post-stage guard digests, all upstream and guarded identity-set
lineage, the nested staging context, and `requirement_count`, `command_count`,
`known_chain_guard_staged_count`, `target_native_not_applicable_count`,
`source_native_not_applicable_count`, `unique_nested_target_count`, and
`total_staged_bytes`. Canonical records and aggregate evidence exclude raw
paths, content, device/inode values, temporary names, and descriptors. Digest
equality and bounded lengths remain visible and potentially guessable.
...
idempotently releases the process-local lease. Its bounded cleanup receipt
distinguishes `removed`, `already_absent_verified`, and `unverifiable`;
uncertain namespace absence or descriptor release fails closed, and neither
staging-root metadata restoration nor secure erasure is reported. This is a
temporary Class 1 byte-staging effect, not authority, authorization, or an
action receipt. It adds no persistence,
proposal/worktree lineage, dispatch, routing, billing, capacity, circuit, live
eligibility, worker, CLI/state/runner integration, subprocess, harness, model,
or execution path. It does not parse or follow the retained target, recurse beyond
depth 2, establish interpreter/`env`/`PATH`/launcher/argument semantics or
dependency/loader/runtime/toolchain closure, close the guard's generic source-
path/staging-root-path-domain or generic-cycle/broader-protected-root gaps, or
prove freshness, atomicity, immutability, authenticity, provenance,
invocability, same-UID/external-writer/fork/hardlink/
...
Implemented as the twenty-second bounded Phase 3 slice, the library-only
schema-v1 Class 0
`ordomata.repository_executable_native_loader_requirements` API exposes
`inspect_staged_executable_native_loader_requirements(expected_runtime, *,
expected_staging, lease)`. It requires the exact direct-executable runtime and
staging receipts and their active same-PID lease, validates complete
correspondence, reproduces runtime evidence before and after inspection,
performs matching bounded extraction passes bracketed by full descriptor
remeasurement, and requires closing lease anchors.

The fixed parser reads only ELF32/ELF64 program-header tables for at most one
`PT_INTERP` and thin Mach-O32/Mach-O64 load-command tables for at most one
`LC_LOAD_DYLINKER`. It performs no fat-image architecture selection. Malformed,
duplicate, fat, and otherwise unsupported layouts receive a fixed
`unsupported_native_layout` disposition; non-native runtime files receive
`non_native_not_applicable`. Supported native records retain only format
class, byte order, image kind, declared/absent disposition, a canonical-
absolute-path fact, digest-only path reference, bounded path byte count, and
exact runtime/staging/command lineage.

Canonical records and evidence exclude raw loader paths, headers, content,
identity numbers, temporary names, and descriptors. This historical Class 0
result adds no authority, authorization, path or loader resolution, shared-
library/dependency closure, write, cleanup, persistence, proposal/worktree,
routing, billing, capacity, circuit, live, network, worker, subprocess,
harness, model, or execution integration. Loader identity, runtime/toolchain
completeness, freshness, authenticity, provenance, invocability, containment,
and future execution remain deferred. Only Class 0/1 effects remain enabled.

Implemented as the twenty-third bounded Phase 3 slice, the library-only
schema-v1 Class 0
`ordomata.repository_executable_native_loader_target_resolution` API exposes
`inspect_staged_executable_native_loader_targets(expected_requirements, *,
expected_runtime, expected_staging, lease, expected_loader_paths)`. It requires
the exact loader-requirements/runtime/staging receipt chain and active same-PID
lease. An exact ordered unique tuple of canonical ASCII absolute paths is
mandatory; each path must recompute the exact digest-only `PT_INTERP` or
`LC_LOAD_DYLINKER` reference for the corresponding staged runtime file before
target measurement starts. Absent, unsupported/fat, and non-native outcomes
bind no path.

Each unique matched target is measured twice through exact-spelling,
component-by-component no-follow traversal. Bounded non-sparse regular
executable files are required. Symlinks, case aliases, duplicate identities,
non-executable files, and content, metadata, ancestor, leaf, or final namespace
drift fail closed. Immutable records preserve only digest-bound path,
filesystem identity, metadata, content, upstream requirement, and command
lineage plus counts and byte totals; raw paths and bytes are excluded.
Deterministic digests remain correlatable and potentially guessable, so this
is privacy minimization rather than secrecy.

This historical Class 0 evidence adds no loader identity/authenticity verdict,
authorization, action receipt, shared-library/dependency closure, architecture
selection, staging, write, cleanup, persistence, proposal/worktree, routing,
billing, capacity, circuit, live, network, worker, subprocess, harness, model,
loader invocation, or execution integration. Freshness, provenance,
...
Implemented as the thirty-ninth bounded Phase 3 slice, the library-only
schema-v1 Class 0
`ordomata.repository_executable_native_dependency_manifest_target_dependency_requirements`
API validates that runtime receipt and active detached lease, then inspects
only bounded ELF `DT_NEEDED` and thin Mach-O dylib-load syntax. Declarations
remain digest-only. It performs no name lookup, pathname open, loading,
...
target staging primitive described above. Exact guarded same-descriptor
capture, active source/target receipt and lease lineage, a protected owner-
private root, unlinked mode-`0400` read-only descriptors, and matching action/
post-stage guard replays retain the measured depth-2 bytes. Native-only input
...
inspection. Bounded ELF `PT_INTERP` and thin Mach-O `LC_LOAD_DYLINKER` syntax
becomes digest-only evidence without loader resolution, shared-library
traversal, fat-image architecture selection, or execution integration.
