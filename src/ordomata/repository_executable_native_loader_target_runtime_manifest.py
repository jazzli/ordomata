"""Read-only runtime-header evidence for one active native-loader-target stage.
...
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_MANIFEST_SCHEMA_VERSION = 1
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_MANIFEST_KIND = (
    "repository_executable_native_loader_target_runtime_manifest"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_MANIFEST_EVIDENCE_KIND = (
    "repository_executable_native_loader_target_runtime_manifest_validation"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_FILE_KIND = (
    "repository_executable_native_loader_target_runtime_file"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_REQUIREMENT_KIND = (
    "repository_executable_native_loader_target_runtime_requirement"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_BINDING_KIND = (
    "repository_executable_native_loader_target_runtime_binding"
)
MANIFEST_SOURCE = "controller_inspected"
MANIFEST_SCOPE = "staged_native_loader_target_runtime_header_v1"

_FIXED_SCHEMA_VERSION = (
    REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_MANIFEST_SCHEMA_VERSION
)
...
_FIXED_FILE_KIND = REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_RUNTIME_FILE_KIND
...
_TARGET_STAGING_KIND = "repository_executable_native_loader_target_staging"
...
_TARGET_STAGING_SCOPE = "native_loader_target_unlinked_readonly_v1"
_TARGET_MEASUREMENT_SOURCE = "controller_measured"
_TARGET_RESOLUTION_SCOPE = (
...
_SOURCE_RUNTIME_CLASSIFICATIONS = (
    "elf",
    "mach_o",
    "posix_shebang",
    "unsupported_shebang",
...
_FIXED_TARGET_STAGE_LEASE_TYPE = RepositoryExecutableNativeLoaderTargetStageLease
...
class RepositoryExecutableNativeLoaderTargetRuntimeFile:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderTargetRuntimeRequirement:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderTargetRuntimeBinding:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderTargetRuntimeManifestReceipt:
    ...
    files: tuple[RepositoryExecutableNativeLoaderTargetRuntimeFile, ...] = field(
        repr=False
    )
    requirements: tuple[
        RepositoryExecutableNativeLoaderTargetRuntimeRequirement, ...
    ] = field(repr=False)
    bindings: tuple[
        RepositoryExecutableNativeLoaderTargetRuntimeBinding, ...
    ] = field(repr=False)
    file_count: int
...
_FIXED_RUNTIME_FILE_TYPE = RepositoryExecutableNativeLoaderTargetRuntimeFile
_FIXED_RUNTIME_REQUIREMENT_TYPE = (
    RepositoryExecutableNativeLoaderTargetRuntimeRequirement
)
_FIXED_RUNTIME_BINDING_TYPE = RepositoryExecutableNativeLoaderTargetRuntimeBinding
...
def _runtime_file_ref_projection(
    *,
    ...
    return {
        ...
        "kind": "repository_executable_native_loader_target_runtime_file_ref",
...
def _runtime_file_projection(
    value: RepositoryExecutableNativeLoaderTargetRuntimeFile,
) -> dict[str, Any]:
    if (
...
def _runtime_requirement_ref_projection(
    *,
    ...
    return {
        ...
        "kind": (
            "repository_executable_native_loader_target_runtime_requirement_ref"
        ),
...
def _runtime_requirement_projection(
    value: RepositoryExecutableNativeLoaderTargetRuntimeRequirement,
) -> dict[str, Any]:
    if (
        type(value)
...
def _runtime_binding_projection(
    value: RepositoryExecutableNativeLoaderTargetRuntimeBinding,
) -> dict[str, Any]:
    if (
    ...
    expected_ref = _BUILTIN_CANONICAL_DIGEST(
        {
            "content_digest": value.content_digest,
            "kind": (
                "repository_executable_native_loader_target_staged_file_ref"
            ),
            "schema_version": 1,
...
def _target_stage_requirement_ref_projection(
    *,
    ...
    return {
        "disposition": disposition,
        "kind": (
            "repository_executable_native_loader_target_stage_requirement_ref"
        ),
        "requirement_ref": requirement_ref,
        "runtime_classification": runtime_classification,
        "runtime_file_ref": runtime_file_ref,
...
def _target_noop_staging_context_digest() -> str:
    return _BUILTIN_CANONICAL_DIGEST(
        {
            "kind": (
                "repository_executable_native_loader_target_staging_context"
            ),
            "schema_version": 1,
...
def _runtime_manifest_projection(
    value: RepositoryExecutableNativeLoaderTargetRuntimeManifestReceipt,
) -> dict[str, Any]:
    digest_fields = (
    ...
    file_by_staged_ref: dict[
        str, RepositoryExecutableNativeLoaderTargetRuntimeFile
    ] = {}
    ...
    requirement_by_ref: dict[
        str, RepositoryExecutableNativeLoaderTargetRuntimeRequirement
    ] = {}
    ...
    for item in value.requirements:
        ...
        stage_requirement_refs.add(item.target_stage_requirement_ref)
        if item.disposition == "declared_loader_target_runtime_inspected":
            direct_count += 1
            if (
    ...
    return {
        ...
        "no_target_requirement_count": (
            value.no_target_requirement_count
        ),
        "registration_digest": value.registration_digest,
        "repository_ref": value.repository_ref,
        "requirement_count": value.requirement_count,
...
def _runtime_manifest_evidence_projection(
    ...
    return {
        ...
        "bounded_native_loader_target_runtime_classification_complete": True,
...
def _root_context_matches(
    expected: RepositoryExecutableNativeLoaderTargetStagingReceipt,
...
def _build_runtime_file(
    ...
    header: bytes,
) -> RepositoryExecutableNativeLoaderTargetRuntimeFile:
...
def _build_runtime_requirement(
    ...
    *,
    ...
    if staged["disposition"] == "declared_loader_target_staged":
        ...
        if runtime_file is None:
            raise _InvalidTargetRuntimeManifest
        disposition = "declared_loader_target_runtime_inspected"
...
def inspect_staged_executable_native_loader_target_runtime_manifest(
    expected_target_staging: (
        RepositoryExecutableNativeLoaderTargetStagingReceipt
    ),
    *,
    ...
    try:
        ...
        receipt = (
            _FIXED_RUNTIME_MANIFEST_RECEIPT_TYPE(
                ...
                no_target_requirement_count=(
                    staging_canonical["no_target_requirement_count"]
                ),
