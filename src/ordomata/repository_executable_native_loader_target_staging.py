"""Ephemeral descriptor staging for direct native-loader-target bytes.
...
copies for later controller inspection.  It does not parse or authenticate the
copied bytes, invoke a loader, execute a command, or grant authority.
...
from .repository_executable_native_loader_target_resolution import (
    ...
    _InvalidNativeLoaderTargetResolution,
    _MeasuredTarget,
...
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_SCHEMA_VERSION = 1
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_KIND = (
    "repository_executable_native_loader_target_staging"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_EVIDENCE_KIND = (
    "repository_executable_native_loader_target_staging_validation"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGED_FILE_KIND = (
    "repository_executable_native_loader_target_staged_file"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_REQUIREMENT_KIND = (
    "repository_executable_native_loader_target_stage_requirement"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_BINDING_KIND = (
    "repository_executable_native_loader_target_stage_binding"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_CLEANUP_KIND = (
    "repository_executable_native_loader_target_stage_cleanup"
)
STAGING_SOURCE = "controller_copied"
STAGING_SCOPE = "native_loader_target_unlinked_readonly_v1"

_INVALID_MESSAGE = "repository executable native loader target staging is invalid"
_CLEANUP_UNCERTAIN_MESSAGE = (
    "repository executable native loader target staging cleanup is uncertain"
)
_COPY_ERROR_MESSAGE = (
    "repository executable native loader target staging lease cannot be copied"
)
...
_BUILTIN_INSPECT_TARGETS = _inspect_staged_executable_native_loader_targets
...
class RepositoryExecutableNativeLoaderTargetStagedFile:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderTargetStageRequirement:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderTargetStageBinding:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderTargetStagingReceipt:
    ...
    staged_files: tuple[
        RepositoryExecutableNativeLoaderTargetStagedFile, ...
    ] = field(repr=False)
    requirements: tuple[
        RepositoryExecutableNativeLoaderTargetStageRequirement, ...
    ] = field(repr=False)
    bindings: tuple[RepositoryExecutableNativeLoaderTargetStageBinding, ...] = field(
        repr=False
    )
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderTargetStageCleanupReceipt:
...
class _RetainedStagedTarget:
    staged_file: RepositoryExecutableNativeLoaderTargetStagedFile = field(
        repr=False
    )
...
class RepositoryExecutableNativeLoaderTargetStageLease:
    """Caller-scoped one-shot lease for detached target byte copies."""

    staging_root: Path = field(repr=False)
    _receipt: RepositoryExecutableNativeLoaderTargetStagingReceipt | None = field(
        default=None,
        init=False,
        repr=False,
    )
    _cleanup_receipt: (
        RepositoryExecutableNativeLoaderTargetStageCleanupReceipt | None
    ...
    _receipt_object_anchor: (
        RepositoryExecutableNativeLoaderTargetStagingReceipt | None
    ) = field(default=None, init=False, repr=False)
    ...
    @property
    def receipt(self) -> RepositoryExecutableNativeLoaderTargetStagingReceipt | None:
    ...
    def cleanup(
        self,
    ) -> RepositoryExecutableNativeLoaderTargetStageCleanupReceipt:
        return cleanup_repository_executable_native_loader_target_stage(self)

    def close(self) -> RepositoryExecutableNativeLoaderTargetStageCleanupReceipt:
        return cleanup_repository_executable_native_loader_target_stage(self)

    def __enter__(self) -> RepositoryExecutableNativeLoaderTargetStageLease:
        if self._state != "active" or self._owner_pid != os.getpid():
            raise ValidationError(_INVALID_MESSAGE)
        return self

    def __exit__(self, *unused: object) -> None:
        cleanup_repository_executable_native_loader_target_stage(self)
...
def _staged_file_projection(
    value: RepositoryExecutableNativeLoaderTargetStagedFile,
) -> dict[str, Any]:
    digest_fields = (
    ...
    if (
        type(value) is not RepositoryExecutableNativeLoaderTargetStagedFile
        or value.kind != REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGED_FILE_KIND
...
def _stage_requirement_ref_projection(
    *,
    ...
    return {
        ...
        "kind": "repository_executable_native_loader_target_stage_requirement_ref",
...
def _stage_requirement_projection(
    value: RepositoryExecutableNativeLoaderTargetStageRequirement,
) -> dict[str, Any]:
    if (
        type(value) is not RepositoryExecutableNativeLoaderTargetStageRequirement
        or value.kind
        != REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_REQUIREMENT_KIND
        or not all(
            _is_digest(item)
        ...
        or (
            value.disposition == "declared_loader_target_staged"
            and (
        ...
        or (
            ...
            and value.runtime_classification not in {"elf", "mach_o"}
        )
        or (
            value.disposition == "non_native_not_applicable"
            and value.runtime_classification in {"elf", "mach_o"}
        )
    ):
        raise _InvalidTargetStaging
...
def _stage_binding_projection(
    value: RepositoryExecutableNativeLoaderTargetStageBinding,
) -> dict[str, Any]:
    if (
        type(value) is not RepositoryExecutableNativeLoaderTargetStageBinding
        or value.kind != REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_BINDING_KIND
...
def _staging_receipt_projection(
    value: RepositoryExecutableNativeLoaderTargetStagingReceipt,
) -> dict[str, Any]:
    digest_fields = (
    ...
    if (
        type(value) is not RepositoryExecutableNativeLoaderTargetStagingReceipt
        or value.kind != REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_KIND
        or type(value.schema_version) is not int
        or value.schema_version
        != REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_SCHEMA_VERSION
    ...
    file_by_ref: dict[str, RepositoryExecutableNativeLoaderTargetStagedFile] = {}
    file_by_measurement: dict[
        str, RepositoryExecutableNativeLoaderTargetStagedFile
    ] = {}
    ...
    requirement_by_ref: dict[
        str, RepositoryExecutableNativeLoaderTargetStageRequirement
    ] = {}
    ...
    for item in value.requirements:
        ...
        if item.disposition == "declared_loader_target_staged":
            direct_count += 1
    ...
    if (
        ...
        or direct_count != value.declared_target_requirement_count
        or native_count != value.no_target_requirement_count
...
def _staging_evidence_projection(
    ...
    return {
        ...
        "bounded_native_loader_target_staging_complete": True,
        ...
        "kind": REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_EVIDENCE_KIND,
        "lease_process_binding_established": True,
        "live_execution_eligible": False,
        "loader_invocation_performed": False,
        "loader_target_bytes_exposed": False,
        "loader_target_path_raw_value_exposed": False,
...
def _cleanup_receipt_projection(
    value: RepositoryExecutableNativeLoaderTargetStageCleanupReceipt,
) -> dict[str, Any]:
    if (
        type(value)
        is not RepositoryExecutableNativeLoaderTargetStageCleanupReceipt
        or value.kind
        != REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_CLEANUP_KIND
        or type(value.schema_version) is not int
        or value.schema_version
...
def _store_cleanup_receipt(
    ...
    receipt: RepositoryExecutableNativeLoaderTargetStageCleanupReceipt,
) -> None:
...
def _require_new_lease(
    ...
    if (
        type(lease) is not RepositoryExecutableNativeLoaderTargetStageLease
...
class _TargetCaptureSink:
    ...
    def __init__(self) -> None:
        self.items: list[_CapturedTarget] = []
    ...
    def __call__(
        ...
        if (
            ...
            or measured.identity in self.identities
            or len(self.items) >= _MAX_TARGETS
        ...
        self.identities.add(measured.identity)
...
def _registration_and_search_context(
    ...
    *,
    ...
    if (
        ...
        is not RepositoryExecutableNativeLoaderRequirementsReceipt
...
def _noop_staging_context_digest() -> str:
    return _BUILTIN_CANONICAL_DIGEST(
        {
            "kind": "repository_executable_native_loader_target_staging_context",
            "schema_version": 1,
...
def _prepare_target_staging_root(
    ...
    *,
    ...
    if (
        ...
        or type(expected_loader_paths) is not tuple
    ):
        raise _InvalidTargetStaging
...
def _new_stage_name() -> str:
    ...
    if (
        ...
        raise _InvalidTargetStaging
    return ".ordomata-native-loader-target-" + value
    ...
    return _BUILTIN_CANONICAL_DIGEST(
        {
            ...
            "kind": (
                "repository_executable_native_loader_target_staged_file_identity"
            ),
            "schema_version": 1,
        }
    )
    ...
    return _BUILTIN_CANONICAL_DIGEST(
        {
            ...
            "kind": (
                "repository_executable_native_loader_target_staged_file_metadata"
            ),
    ...
    return _BUILTIN_CANONICAL_DIGEST(
        {
            ...
            "kind": "repository_executable_native_loader_target_staged_file_ref",
            "schema_version": 1,
            "source_filesystem_identity_ref": (
...
def _mark_cleanup_unverifiable(
    ...
    receipt = RepositoryExecutableNativeLoaderTargetStageCleanupReceipt(
        kind=REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_CLEANUP_KIND,
        schema_version=(
            REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_SCHEMA_VERSION
        ),
...
def _stage_captured_target(
    ...
    *,
    ...
    try:
        ...
        staged_file = RepositoryExecutableNativeLoaderTargetStagedFile(
            kind=REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGED_FILE_KIND,
            target_path_ref=captured.target_path_ref,
            source_filesystem_identity_ref=(
                captured.source_filesystem_identity_ref
            ),
...
def _build_staging_receipt(
    *,
    expected: RepositoryExecutableNativeLoaderTargetResolutionReceipt,
    action: RepositoryExecutableNativeLoaderTargetResolutionReceipt,
    post_stage: RepositoryExecutableNativeLoaderTargetResolutionReceipt,
    ...
    requirements: list[RepositoryExecutableNativeLoaderTargetStageRequirement] = []
    for upstream in action.requirements:
        if upstream.target_disposition == "declared_loader_target_measured":
            ...
            if staged is None:
                raise _InvalidTargetStaging
            disposition = "declared_loader_target_staged"
            target_measurement_ref = upstream.target_measurement_ref
            target_staged_file_ref = staged.target_staged_file_ref
        elif upstream.target_disposition in {
            "loader_declaration_absent",
            "unsupported_native_layout",
            "non_native_not_applicable",
        }:
            disposition = upstream.target_disposition
        ...
        reference = _stage_requirement_ref_projection(
            ...
            requirement_ref=upstream.native_loader_requirement_ref,
        ...
        requirement = RepositoryExecutableNativeLoaderTargetStageRequirement(
            kind=(
                REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_REQUIREMENT_KIND
            ),
            staged_file_ref=upstream.staged_file_ref,
    ...
    bindings: list[RepositoryExecutableNativeLoaderTargetStageBinding] = []
    for upstream in action.bindings:
        ...
        if requirement is None:
            raise _InvalidTargetStaging
        binding = RepositoryExecutableNativeLoaderTargetStageBinding(
            kind=REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGE_BINDING_KIND,
    ...
    receipt = RepositoryExecutableNativeLoaderTargetStagingReceipt(
        kind=REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_KIND,
        schema_version=(
            REPOSITORY_EXECUTABLE_NATIVE_LOADER_TARGET_STAGING_SCHEMA_VERSION
        ),
        ...
        staging_scope=STAGING_SCOPE,
        native_loader_requirements_receipt_digest=(
            action.native_loader_requirements_receipt_digest
        ),
        runtime_manifest_receipt_digest=(
        ...
        loader_path_context_digest=action.loader_path_context_digest,
        ...
        declared_target_requirement_count=(
            action.declared_target_requirement_count
        ),
        no_target_requirement_count=action.no_target_requirement_count,
...
def _abort_staging(
    ...
    if type(lease) is not RepositoryExecutableNativeLoaderTargetStageLease:
        return True
...
def cleanup_repository_executable_native_loader_target_stage(
    lease: RepositoryExecutableNativeLoaderTargetStageLease,
) -> RepositoryExecutableNativeLoaderTargetStageCleanupReceipt:
...
def _captures_in_action_order(
    ...
    if (
        len(capture.items) != action.unique_target_count
        or capture.total_bytes != action.total_measured_bytes
    ):
        raise _InvalidTargetStaging
    ordered: list[_CapturedTarget] = []
    for captured, measurement in zip(
        capture.items,
        action.measurements,
        strict=True,
    ):
        if (
            captured.content_digest != measurement.content_digest
        ...
        ordered.append(
            _CapturedTarget(
                target_path_ref=measurement.path_ref,
...
def stage_repository_executable_native_loader_target_bytes(
    registration: RepositoryRegistration,
    *,
    search_directories: tuple[Path, ...],
    expected_target_resolution: (
    ...
    try:
        ...
        if action.unique_target_count == 0:
            if (
                action.declared_target_requirement_count != 0
...
__all__ = [
    ...
    "STAGING_SCOPE",
    "STAGING_SOURCE",
    "RepositoryExecutableNativeLoaderTargetStageBinding",
    "RepositoryExecutableNativeLoaderTargetStageCleanupReceipt",
    "RepositoryExecutableNativeLoaderTargetStageLease",
    "RepositoryExecutableNativeLoaderTargetStageRequirement",
    "RepositoryExecutableNativeLoaderTargetStagedFile",
