"""Ephemeral descriptor staging for guarded nested native-loader target bytes.
...
from .repository_executable_native_loader_nested_target_chain_guard import (
    GUARD_SCOPE,
    INSPECTION_SOURCE,
...
from .repository_executable_native_loader_nested_target_resolution import (
    ...
    _public_measurement as _native_public_measurement,
...
REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGING_SCHEMA_VERSION = 1
REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGING_KIND = (
    "repository_executable_native_loader_nested_target_staging"
)
REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGING_EVIDENCE_KIND = (
    "repository_executable_native_loader_nested_target_staging_validation"
)
...
REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGE_REQUIREMENT_KIND = (
...
REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGE_CLEANUP_KIND = (
    "repository_executable_native_loader_nested_target_stage_cleanup"
)
STAGING_SOURCE = "controller_copied"
STAGING_SCOPE = "native_loader_nested_target_unlinked_readonly_v1"

_INVALID_MESSAGE = (
    "repository executable native loader nested target staging is invalid"
)
_CLEANUP_UNCERTAIN_MESSAGE = (
    "repository executable native loader nested target staging cleanup is uncertain"
)
_COPY_ERROR_MESSAGE = (
...
_BUILTIN_NATIVE_PUBLIC_MEASUREMENT = _native_public_measurement
...
_FIXED_TARGET_STAGING_TYPE = RepositoryExecutableNativeLoaderTargetStagingReceipt
...
class RepositoryExecutableNativeLoaderNestedTargetStagedFile:
    """One guarded nested measurement and its detached byte copy."""

    kind: str
    nested_loader_path_ref: str = field(repr=False)
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderNestedTargetStageRequirement:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderNestedTargetStageLineage:
    """One source lineage bound through nested-target staging evidence."""
    ...
    nested_target_stage_requirement_ref: str | None = field(repr=False)
    disposition: str
    nested_target_stage_lineage_ref: str = field(repr=False)

    def to_canonical(self) -> dict[str, Any]:
        return _stage_lineage_projection(self)


@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderNestedTargetStageBinding:
    """One command bound through one nested-target staging lineage."""
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderNestedTargetStagingReceipt:
...
@dataclass(frozen=True, slots=True)
class RepositoryExecutableNativeLoaderNestedTargetStageCleanupReceipt:
...
class _RetainedStagedNestedTarget:
    staged_file: RepositoryExecutableNativeLoaderNestedTargetStagedFile = field(
        repr=False
    )
...
class RepositoryExecutableNativeLoaderNestedTargetStageLease:
    """Caller-scoped one-shot lease for detached nested-target bytes."""

    staging_root: Path = field(repr=False)
    _receipt: RepositoryExecutableNativeLoaderNestedTargetStagingReceipt | None = (
        field(default=None, init=False, repr=False)
    )
    _cleanup_receipt: (
        RepositoryExecutableNativeLoaderNestedTargetStageCleanupReceipt | None
    ) = field(default=None, init=False, repr=False)
    _receipt_digest_anchor: str | None = field(
        default=None, init=False, repr=False
    )
    _receipt_object_anchor: (
        RepositoryExecutableNativeLoaderNestedTargetStagingReceipt | None
    ) = field(default=None, init=False, repr=False)
    ...
    def cleanup(
        ...
        return cleanup_repository_executable_native_loader_nested_target_stage(self)
    ...
    def __exit__(self, *unused: object) -> None:
        cleanup_repository_executable_native_loader_nested_target_stage(self)
...
_FIXED_LEASE_TYPE = RepositoryExecutableNativeLoaderNestedTargetStageLease
...
def _staged_file_ref_projection(
    *,
    ...
    return {
        ...
        "kind": (
            "repository_executable_native_loader_nested_target_staged_file_ref"
        ),
        "nested_target_measurement_ref": nested_target_measurement_ref,
        "nested_loader_path_ref": nested_loader_path_ref,
        "nested_target_staging_context_digest": (
            nested_target_staging_context_digest
        ),
        "schema_version": (
            REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGING_SCHEMA_VERSION
        ),
...
def _staged_file_projection(
    value: RepositoryExecutableNativeLoaderNestedTargetStagedFile,
    *,
    ...
    if staging_context_digest is not None:
        reference = _staged_file_ref_projection(
            nested_loader_path_ref=value.nested_loader_path_ref,
            source_filesystem_identity_ref=(
                value.source_filesystem_identity_ref
            ),
...
def _stage_requirement_projection(
    value: RepositoryExecutableNativeLoaderNestedTargetStageRequirement,
) -> dict[str, Any]:
    digests = (
...
def _stage_lineage_ref_projection(
    *,
    chain_guard_lineage_ref: str,
    nested_target_requirement_ref: str | None,
    chain_guard_requirement_ref: str | None,
    nested_target_stage_requirement_ref: str | None,
    disposition: str,
) -> dict[str, Any]:
    bound = disposition == "stage_requirement_bound"
    optional_refs = (
    ...
    if (
        not _is_digest(chain_guard_lineage_ref)
        or disposition not in _LINEAGE_DISPOSITIONS
        or any(item is not None and not _is_digest(item) for item in optional_refs)
        or bound != all(item is not None for item in optional_refs)
        or (not bound and any(item is not None for item in optional_refs))
    ):
        raise _InvalidNestedTargetStaging
    return {
...
def _stage_lineage_projection(
    value: RepositoryExecutableNativeLoaderNestedTargetStageLineage,
) -> dict[str, Any]:
    digests = (
    ...
    if (
        type(value) is not _FIXED_STAGE_LINEAGE_TYPE
        or value.kind
        != REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGE_LINEAGE_KIND
        or not all(_is_digest(item) for item in digests)
    ):
        raise _InvalidNestedTargetStaging
    reference = _stage_lineage_ref_projection(
    ...
    if value.nested_target_stage_lineage_ref != _BUILTIN_CANONICAL_DIGEST(
        reference
    ):
        raise _InvalidNestedTargetStaging
    return {
...
def _stage_binding_projection(
    value: RepositoryExecutableNativeLoaderNestedTargetStageBinding,
) -> dict[str, Any]:
    lineage = (
...
def _staging_receipt_projection(
    value: RepositoryExecutableNativeLoaderNestedTargetStagingReceipt,
) -> dict[str, Any]:
    digest_fields = (
    ...
    lineages = [_stage_lineage_projection(item) for item in value.lineages]
    ...
    lineages_by_guard: dict[
        str,
        RepositoryExecutableNativeLoaderNestedTargetStageLineage,
    ] = {}
    stage_lineage_refs: set[str] = set()
    bound_requirements: set[str] = set()
    for item in value.lineages:
        if (
            item.chain_guard_lineage_ref in lineages_by_guard
            or item.nested_target_stage_lineage_ref in stage_lineage_refs
        ):
            raise _InvalidNestedTargetStaging
        if item.disposition == "stage_requirement_bound":
            if item.chain_guard_requirement_ref is None:
                raise _InvalidNestedTargetStaging
            expected_requirement = requirements_by_guard.get(
                item.chain_guard_requirement_ref
            )
            if (
                expected_requirement is None
                or item.nested_target_requirement_ref
                != expected_requirement.nested_target_requirement_ref
                or item.nested_target_stage_requirement_ref
                != expected_requirement.nested_target_stage_requirement_ref
            ):
                raise _InvalidNestedTargetStaging
            bound_requirements.add(item.chain_guard_requirement_ref)
        lineages_by_guard[item.chain_guard_lineage_ref] = item
        stage_lineage_refs.add(item.nested_target_stage_lineage_ref)
    if bound_requirements != set(requirements_by_guard):
        raise _InvalidNestedTargetStaging
    ...
    for item in value.bindings:
        ...
        if (
            ...
            != expected.target_loader_lineage_ref
        ):
            raise _InvalidNestedTargetStaging
        command_ids.add(item.command_id)
        bound_lineages.add(item.chain_guard_lineage_ref)
        prior_kind_index = kind_index
    if bound_lineages != set(lineages_by_guard):
        raise _InvalidNestedTargetStaging
...
def _staging_evidence_projection(
    ...
    return {
        ...
        "loader_declaration_absent_count": canonical[
            "loader_declaration_absent_count"
        ],
        ...
        "non_native_not_applicable_count": canonical[
            "non_native_not_applicable_count"
        ],
        ...
        "unsupported_native_layout_count": canonical[
            "unsupported_native_layout_count"
        ],
...
def _cleanup_receipt_projection(
    value: RepositoryExecutableNativeLoaderNestedTargetStageCleanupReceipt,
) -> dict[str, Any]:
    if (
...
class _NestedTargetCaptureSink:
    ...
    def __call__(
        ...
        if (
            ...
            raise _InvalidNestedTargetStaging
        public_measurement = _BUILTIN_NATIVE_PUBLIC_MEASUREMENT(measured)
        if public_measurement.path_ref in self.by_path_ref:
            raise _InvalidNestedTargetStaging
        try:
        ...
        if (
            ...
            raise _InvalidNestedTargetStaging
        if public_measurement.content_digest != content_digest:
            raise _InvalidNestedTargetStaging
        self.by_path_ref[public_measurement.path_ref] = _FIXED_CAPTURED_TYPE(
            nested_loader_path_ref=public_measurement.path_ref,
            source_identity=measured.identity,
            source_filesystem_identity_ref=(
                public_measurement.filesystem_identity_ref
            ),
            source_metadata=measured.metadata,
            source_metadata_digest=public_measurement.metadata_digest,
            content_digest=content_digest,
...
def _registration_and_search_context(
    ...
    *,
    ...
    expected_nested: RepositoryExecutableNativeLoaderNestedTargetResolutionReceipt,
...
def _prepare_staging_root(
    ...
    *,
    ...
    if (
        ...
        or type(expected_nested_loader_paths) is not tuple
        or any(
            type(path) is not _CONCRETE_PATH_TYPE
            for path in (*expected_loader_paths, *expected_nested_loader_paths)
        )
...
def _new_stage_name() -> str:
    ...
    if (
        ...
        raise _InvalidNestedTargetStaging
    return ".ordomata-native-loader-nested-target-" + value
...
def _stage_captured_nested_target(
    ...
    *,
    ...
    try:
        ...
        reference = _staged_file_ref_projection(
            nested_loader_path_ref=captured.nested_loader_path_ref,
            source_filesystem_identity_ref=(
...
def _captures_in_action_order(
    ...
    action_guard: RepositoryExecutableNativeLoaderNestedTargetChainGuardReceipt,
    expected_nested: RepositoryExecutableNativeLoaderNestedTargetResolutionReceipt,
) -> tuple[
    tuple[
    ...
    for guarded in action_guard.guarded_measurements:
        ...
        if (
            captured is None
            or captured.nested_loader_path_ref in seen_paths
            ...
            raise _InvalidNestedTargetStaging
        seen_paths.add(captured.nested_loader_path_ref)
        ordered.append(
            (
                captured,
                guarded.nested_target_measurement_ref,
    ...
    *,
    ...
    for upstream in action_guard.requirements:
        ...
        requirement = _FIXED_STAGE_REQUIREMENT_TYPE(
            ...
            target_loader_requirement_ref=(
                upstream.target_loader_requirement_ref
            ),
            nested_target_requirement_ref=(
    ...
    for upstream in action_guard.lineages:
        if upstream.disposition == "guard_requirement_bound":
            if upstream.chain_guard_requirement_ref is None:
                raise _InvalidNestedTargetStaging
            ...
            if (
                requirement is None
                or upstream.nested_target_requirement_ref
                != requirement.nested_target_requirement_ref
            ):
                raise _InvalidNestedTargetStaging
            disposition = "stage_requirement_bound"
            stage_requirement_ref = (
            ...
            if (
                upstream.nested_target_requirement_ref is not None
                or upstream.chain_guard_requirement_ref is not None
            ):
                raise _InvalidNestedTargetStaging
            disposition = upstream.disposition
            stage_requirement_ref = None
        else:
            raise _InvalidNestedTargetStaging
        reference = _stage_lineage_ref_projection(
            ...
            nested_target_stage_requirement_ref=stage_requirement_ref,
            disposition=disposition,
        )
        lineage = _FIXED_STAGE_LINEAGE_TYPE(
            kind=(
                REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGE_LINEAGE_KIND
            ),
        ...
        _stage_lineage_projection(lineage)
        lineages.append(lineage)

    lineage_by_guard = {
        item.chain_guard_lineage_ref: item for item in lineages
    }
    bindings: list[RepositoryExecutableNativeLoaderNestedTargetStageBinding] = []
    for upstream in action_guard.bindings:
        lineage = lineage_by_guard.get(upstream.chain_guard_lineage_ref)
        if lineage is None:
            raise _InvalidNestedTargetStaging
    ...
    receipt = _FIXED_STAGING_RECEIPT_TYPE(
        kind=REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGING_KIND,
        schema_version=(
            REPOSITORY_EXECUTABLE_NATIVE_LOADER_NESTED_TARGET_STAGING_SCHEMA_VERSION
        ),
        ...
        target_loader_requirements_receipt_digest=(
            action_guard.target_loader_requirements_receipt_digest
        ),
        target_runtime_manifest_receipt_digest=(
        ...
        native_loader_requirements_receipt_digest=(
            action_guard.native_loader_requirements_receipt_digest
        ),
        runtime_manifest_receipt_digest=(
        ...
        first_loader_path_context_digest=action_guard.first_loader_path_context_digest,
        target_staging_context_digest=(
            action_guard.target_staging_context_digest
        ),
        nested_loader_path_context_digest=(
            action_guard.nested_loader_path_context_digest
        ),
        known_source_identity_set_digest=(
...
def _store_cleanup_receipt(
    lease: RepositoryExecutableNativeLoaderNestedTargetStageLease,
    receipt: RepositoryExecutableNativeLoaderNestedTargetStageCleanupReceipt,
) -> None:
...
def cleanup_repository_executable_native_loader_nested_target_stage(
    lease: RepositoryExecutableNativeLoaderNestedTargetStageLease,
) -> RepositoryExecutableNativeLoaderNestedTargetStageCleanupReceipt:
...
def stage_repository_executable_native_loader_nested_target_bytes(
    registration: RepositoryRegistration,
    *,
    search_directories: tuple[Path, ...],
