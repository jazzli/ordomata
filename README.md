# Subscription Agent Orchestrator

A local, single-operator control plane for running neutral tasks through first-party AI coding-harness subscriptions without using purchased product credits, subscription overage, separately billed model APIs, or cloud inference routes. The current tracer bullet is **Chief of Staff Lite**: sanitized local sources become an immutable context snapshot, a structured local draft, and transparent deterministic evaluation.

Source, tests, plans, and sanitized configuration can be tracked in a private GitHub repository under the operator's `jazzli` account for provenance and portability. Local state, attestations, logs, workspaces, and artifacts remain ignored and are not pushed automatically.
...
- read-only supervisor status/audit, digest-bound reconciliation preview/apply,
  explicit control commands, and a foreground `agentops supervise` loop whose
...
```sh
PYTHONPATH=src python3 -m agentops task-validate
PYTHONPATH=src python3 -m agentops context-inspect
PYTHONPATH=src python3 -m agentops profiles
PYTHONPATH=src python3 -m agentops route --lane mock
PYTHONPATH=src python3 -m agentops demo
PYTHONPATH=src python3 -m agentops auth-inspect
```

The demo does not invoke a model. It writes its accepted artifact and append-only state under `.agentops/`, which is ignored by Git.

`auth-inspect` never initializes or changes the state database. With no state it
returns a clean empty report without creating `.agentops`; after a run it can
...
```sh
PYTHONPATH=src python3 -m agentops doctor
PYTHONPATH=src python3 -m agentops route --lane subscription
```
...
```sh
PYTHONPATH=src python3 -m agentops billing-attest --runner codex
PYTHONPATH=src python3 -m agentops billing-attest --runner claude
PYTHONPATH=src python3 -m agentops doctor
```
...
```sh
PYTHONPATH=src python3 -m agentops compare-plan \
...
```sh
PYTHONPATH=src python3 -m agentops schedule-inspect \
...
```sh
PYTHONPATH=src python3 -m agentops supervisor status --json
PYTHONPATH=src python3 -m agentops supervisor audit --json
PYTHONPATH=src python3 -m agentops supervisor reconcile --json
```

Exercise its mock-only control plane explicitly:

```sh
PYTHONPATH=src python3 -m agentops supervisor enqueue \
  --admission-key demo-flow --flow-id demo-flow --json
PYTHONPATH=src python3 -m agentops supervisor start --json
PYTHONPATH=src python3 -m agentops supervise --once --json
PYTHONPATH=src python3 -m agentops supervisor pause --json
PYTHONPATH=src python3 -m agentops supervisor stop --json
```
...
7. the operator explicitly sets `AGENTOPS_ALLOW_SUBSCRIPTION_RUNS=1` for that process.
...
PYTHONPATH=src python3 -m agentops run \
...
See [architecture](docs/architecture.md), [runtime authorization model](docs/authorization-model.md), [billing policy](docs/subscription-only-policy.md), [routing](docs/routing.md), [roadmap](docs/roadmap.md), and [implementation status](docs/implementation-status.md).
